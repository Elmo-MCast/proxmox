...
num_servers = len(vm_ids['web_server'])
num_clients = len(vm_ids['faban_client'])

ids = dict()
ids['lb_server_%s' % (vm_ids['lb_server'],)] = vm_ids['lb_server']
ids['db_server_%s' % (vm_ids['db_server'],)] = vm_ids['db_server']
ids['memcached_server_%s' % (vm_ids['memcached_server'],)] = vm_ids['memcached_server']
...
def configure():
    configure_db_server(vm_ids['db_server'], vm_ids['lb_server'])
    configure_memcached_server(vm_ids['memcached_server'])
    for i in range(num_servers):
        configure_web_server(vm_ids['web_server'][i], vm_ids['db_server'],
                             vm_ids['memcached_server'], pm_max_childs)
