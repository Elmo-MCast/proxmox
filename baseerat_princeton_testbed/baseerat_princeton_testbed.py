...
def pve_cleanup_vswitch(bridge_name, ports, execute=True):
    script = ""
    for port in ports:
...
def pve_setup_vswitch(cpu_mask, bridge_name, ports, execute=True):
    script = ovs_stop(False)
...
def pve2_setup_clients():
    pve.vm_generate_multi(fab.env['pu-pve2']['settings']['vm']['base_id'], 'client', False, None,
                          *_get_vm_id_list(fab.env['pu-pve2']['clients']['vms']))
...
def pve2_cleanup_clients():
    pve.vm_destroy_multi(*_get_vm_id_list(fab.env['pu-pve2']['clients']['vms']))


''' pu-pve1 (End-host and LB) Commands '''


@fab.roles('pu-pve1')
def pve1_setup_vswitch():
    bridge_name = fab.env['pu-pve1']['bridge']['name']
    ports = fab.env['pu-pve1']['bridge']['ports']
    bridge_1 = fab.env['pu-pve1']['settings']['vm']['bridge_1']
    script = pve_setup_vswitch(0x1, bridge_name, ports, False)
    script += pve_connect_vswitch_to_bridge_1(bridge_name, bridge_1, False)
    script += ovs_show_bridge('ofctl', bridge_name, False)
    fab.run(script)
...
@fab.roles('pu-pve1')
def pve1_configure_flow_rules():
    bridge_name = fab.env['pu-pve1']['bridge']['name']
    flow_rules = fab.env['pu-pve1']['bridge']['flow_rules']
    script = ""
    for flow_rule in flow_rules:
        script += ovs_add_flow(bridge_name, flow_rule, False)
    fab.run(script)
...
@fab.roles('pu-pve1')
def pve1_setup_lb():
    pve.vm_generate_multi(fab.env['pu-pve1']['settings']['vm']['base_id'], 'lb', False, None,
                          *_get_vm_id_list([fab.env['pu-pve1']['lb']['vm']]))
    _setup_options([fab.env['pu-pve1']['lb']['vm']],
                   fab.env['pu-pve1']['lb']['options'])
    scripts = _setup_ifaces(None, [fab.env['pu-pve1']['lb']['vm']],
                            fab.env['pu-pve1']['settings']['vm'], False)
    # TODO: add scripts once we know what'd be running on these VMs.
    pve.vm_parallel_run(scripts)
