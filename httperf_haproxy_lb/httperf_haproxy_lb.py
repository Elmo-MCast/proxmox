...
with open(os.path.dirname(__file__) + "/httperf_haproxy_lb.json") as json_file:
...
fab.env['httperf_haproxy_lb'] = settings['httperf_haproxy_lb']

""" Helper Functions """

vm_id_set = set()
for server_name, server_configs in fab.env['httperf_haproxy_lb']['servers'].iteritems():
...
def configure_state_servers():
    scripts = dict()
    for state_server in fab.env['httperf_haproxy_lb']['servers']['state_server']:
